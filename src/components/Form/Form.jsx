...
const Control = ({as = "input", className = "", plaintext = false, id="", type = "text", autoFocus=false, ...restProps}, ref) => {
...
const Text = ({children, className, ...restProps}, ref) => {
    return (
        <small className={`sg-form-text${className ? " "+className:""}`}>
            {children}
        </small>
    )
}
Form.Text = forwardRef(Text)
